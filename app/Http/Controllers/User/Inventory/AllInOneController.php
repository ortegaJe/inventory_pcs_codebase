...
class AllInOneController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $deviceType             = TypeDevice::select('name as type_name')->where('id', TypeDevice::ALL_IN_ONE_PC_ID)->first();

        if ($request->ajax()) {

            $devices = DB::table('view_all_devices')
                ->where('TipoPc', TypeDevice::EQUIPOS_ALL_IN_ONES)
        ...
        if ($validator->fails()) :
            ...
            DB::insert(
                ...
                [
                    ...
                    $this->device->type_device_id = TypeDevice::ALL_IN_ONE_PC_ID,
