      ...
      <a class="block block-rounded block-link-shadow" href="{{ route('user.inventory.laptop.create') }}">
        ...
        <div class="block block-rounded block-bordered">
            <div class="block-header block-header-default border-b">
                ...
                <div class="block-options">
                    <button type="button" class="btn btn-alt-success" data-toggle="tooltip" data-placement="top"
                        title="Nuevo equipo {{ Str::lower($deviceType->type_name) }}"
                        onclick="window.location='{{ route('tablet.create') }}'">
                        <i class="fa fa-plus"></i>
                    </button>
                    <button type="button" class="btn btn-alt-primary" id="btn-refresh1" data-toggle="tooltip"
                        data-placement="top" title="Actualizar lista">
...
@push('js')
    <script src="{{ asset('/js/datatables/datatable.inventory.tablet.js') }}"></script>
    <script src="{{ asset('/js/pages/be_tables_datatables.min.js') }}"></script>
    <script src="{{ asset('/js/plugins/datatables/jquery.dataTables.min.js') }}"></script>
    <script src="{{ asset('/js/plugins/datatables/dataTables.bootstrap4.min.js') }}"></script>

    <script>
        let root_url_tablet = <?php echo json_encode(route('tablet.index')); ?>;
        let root_url_tablet_store = <?php echo json_encode(route('tablet.store')); ?>;
    </script>
